...
final class TestHarness: ObservableObject {
    ...
    func runTest(
        ...
        do {
            result = try await inferenceEngine.run(
                ...
                onProgress: { [weak self] progress in
                    Task { @MainActor [weak self] in
                        ...
                        case .loading: self.statusMessage = "Loading model..."
                        case .generating: self.statusMessage = "Generating..."
            ...
            return InferenceResult(
                id: UUID(), modelId: model.id, modelName: model.fileName,
                ...
                timeToFirstTokenMs: 0, tokensPerSecond: 0, peakMemoryMB: 0,
                timestamp: Date(), didFail: true, errorMessage: error.localizedDescription
            )
        }
        ...
        let finalResult = InferenceResult(
            id: result.id, modelId: result.modelId, modelName: result.modelName,
            inputTokenCount: result.inputTokenCount, outputTokenCount: result.outputTokenCount,
            articleText: result.articleText, summary: result.summary,
            latencyMs: result.latencyMs, timeToFirstTokenMs: result.timeToFirstTokenMs,
            tokensPerSecond: result.tokensPerSecond, peakMemoryMB: max(beforeMem, afterMem),
            timestamp: result.timestamp, didFail: result.didFail, errorMessage: result.errorMessage
        )
    ...
    func loadResults() async -> [InferenceResult] { await metricsStore.loadAll() }
    func clearResults() async { await metricsStore.clearAll() }
    func exportCSV() async -> URL { await metricsStore.exportCSV() }
}
